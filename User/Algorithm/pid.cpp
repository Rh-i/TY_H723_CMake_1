...
_PID_Param_t::_PID_Param_t(float kp, float ki, float kd)
    : kp(kp), ki(ki), kd(kd) {}
_PID_Limitation_t::_PID_Limitation_t()
    : Out_max(0), P_max(0), I_max(0), D_max(0), F_max(0), I_separation(0) {}
_PID_Limitation_t::_PID_Limitation_t(float Out_max, float P_max, float I_max,
                                     float D_max, float F_max,
                                     float I_separation)
    : Out_max(Out_max), P_max(P_max), I_max(I_max), D_max(D_max), F_max(F_max),
      I_separation(I_separation) {
  this->I_max = I_max == 0 ? Out_max : I_max;
}
_PID_Term_t::_PID_Term_t() : P_term(0), I_term(0), D_term(0), F_term(0) {}

PID_t::PID_t(_PID_Param_t param, _PID_Limitation_t limitation)
    : param(param), lim(limitation), term() {
...
PID_t::PID_t(float kp, float ki, float kd, float Out_max, float P_max,
             float I_max, float D_max, float F_max, float I_separation)
    : param(kp, ki, kd), lim(Out_max, P_max, I_max, D_max, F_max, I_separation),
      term() {
  diffCalcMode = param.kd != 0 ? Diff_target : Disable_PID_Diff;
}
