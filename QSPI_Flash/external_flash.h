...
typedef enum {
    ...
    EXT_FLASH_ERROR_DEVICE_ID = -2, /**< JEDEC ID 与 W25Q256 不匹配。 */
...
/**
 * @brief 复位、识别并初始化 W25Q256。
 *
 * 应在 `MX_QUADSPI_Init()` 之后、任何读写操作之前调用一次。函数会安装
 * QUADSPI 中断、复位器件、读取 JEDEC ID 和状态寄存器，并确保 QE 位有效。
 *
...
/**
 * @brief 动态开放实际 32 MiB QSPI 地址窗口的 MPU Region 2。
...
/**
 * @brief 禁用 MPU Region 2，使 Region 1 的 QSPI No-Access 保护重新生效。
 */
