...
#define FLASH_DEVICE_EXPECTED_CAPACITY_ID     0x19U /**< 256 Mbit 容量 ID。 */
#define FLASH_DEVICE_SR1_BUSY                 0x01U /**< SR1 写入/擦除忙标志。 */
#define FLASH_DEVICE_SR1_WEL                  0x02U /**< SR1 写使能锁存标志。 */
#define FLASH_DEVICE_SIZE                    0x02000000UL /**< 容量：32 MiB。 */
#define FLASH_DEVICE_PAGE_SIZE               256U /**< Page Program 页大小。 */
#define FLASH_DEVICE_SECTOR_SIZE             4096U /**< 最小擦除扇区大小。 */

/** @brief W25Q256 JEDEC 标识和三个状态寄存器的快照。 */
typedef struct {
    uint8_t manufacturer_id; /**< JEDEC 厂商 ID，W25Q256 应为 0xEF。 */
    uint8_t memory_type; /**< JEDEC 器件类型，W25Q256 应为 0x40。 */
    uint8_t capacity_id; /**< JEDEC 容量码，W25Q256 应为 0x19。 */
...
/** @brief 检查信息是否对应本驱动支持的 W25Q256。 @param[in] info 待检查信息。 @return 匹配返回非 0，否则返回 0。 */
...
/** @brief 用 4-byte Read 命令读取。 @param[in] address 器件内部偏移。 @param[out] data 接收缓冲区。 @param[in] length 字节数。 @return 0 成功，-1 失败。 */
int flash_device_read(uint32_t address, void *data, uint32_t length);

/** @brief 用 4-byte Page Program 写一页内的数据。 @param[in] address 起始偏移。 @param[in] data 源数据。 @param[in] length 1~256 字节且不得跨页。 @return 0 成功，负值失败。 */
int flash_device_program_page(uint32_t address, const void *data, uint32_t length);

/** @brief 用 4-byte 4 KiB Erase 擦除对齐扇区。 @param[in] address 4 KiB 对齐偏移。 @return 0 成功，负值失败。 */
...
/** @brief 使用 MDMA 和 4-byte Read 读取。参数含义同 flash_device_read()。 @return 0 成功，-1 失败。 */
...
/** @brief 配置 4-byte Quad Output Fast Read Memory-Mapped 模式。 @return 0 成功，-1 失败。 */
