...
/** @brief 开启 QUADSPI IRQ，必须在使用 MDMA 异步 HAL 接口前调用。 */
...
/** @brief 发送带 32 位地址、无数据命令。 @param[in] instruction 指令码。 @param[in] address 器件内部地址。 @return 0 成功，-1 失败。 */
int flash_port_address_command(uint8_t instruction, uint32_t address);

/** @brief 发送指令和 32 位地址后轮询接收。 @param[in] instruction 指令码。 @param[in] address 器件内部地址。 @param[out] data 接收缓冲区。 @param[in] length 字节数。 @return 0 成功，-1 失败。 */
int flash_port_address_receive(uint8_t instruction, uint32_t address,
                               void *data, size_t length);

/** @brief 发送指令和 32 位地址后轮询发送。 @param[in] instruction 指令码。 @param[in] address 器件内部地址。 @param[in] data 源缓冲区。 @param[in] length 字节数。 @return 0 成功，-1 失败。 */
...
/** @brief 配置 QUADSPI Memory-Mapped 读取。 @param[in] instruction 读取指令。 @param[in] dummy_cycles 空周期数。 @return 0 成功，-1 失败。 */
int flash_port_memory_mapped_enable(uint8_t instruction, uint8_t dummy_cycles);

/** @brief 通过 HAL_QSPI_Abort() 退出 Memory-Mapped 模式。 @return 0 成功，-1 失败。 */
int flash_port_memory_mapped_disable(void);

/** @brief 配置 MPU Region 2 开放 32 MiB 窗口。 @param[in] executable 非 0 允许取指，0 禁止取指。 @return 0。 */
int flash_port_memory_mapped_access_enable(int executable);

/** @brief 禁用 MPU Region 2，恢复上层 No-Access 保护。 */
